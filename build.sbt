...
lazy val `compaas-bench` = project
  ...
  .settings(
    libraryDependencies ++=
      Seq("com.typesafe.akka" %% "akka-actor-typed" % versions.Akka).map(_.cross(CrossVersion.for3Use2_13)),
    libraryDependencies ++=
      Seq(
    ...
    Compile / resourceGenerators +=
      Def
        .task {
          ...
          val wasmFiles: Seq[File] = (root.base / "target" / "wasm32-unknown-unknown" / "release")
            .listFiles
