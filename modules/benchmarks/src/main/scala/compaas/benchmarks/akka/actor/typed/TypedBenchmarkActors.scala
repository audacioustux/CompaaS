...
object TypedBenchmarkActors:
  ...
  private def echoSender(
      ...
      batchSize: Int,
      childProps: Props,
  ): Behavior[Message.type] = Behaviors.setup { ctx =>
  ...
  def benchmarkPingPongSupervisor(): Behavior[PingPongCommand] = Behaviors.setup { ctx =>
  ...
  end startPingPongActorPairs

  case class Message(replyTo: ActorRef[Message])

  private def newPingPongBehavior(messagesPerPair: Int, latch: CountDownLatch): Behavior[Message] = Behaviors
    .setup { ctx =>
      var left = messagesPerPair / 2
