...
object NthPrimeBenchmark:
  val modules = Map(
    "js" ->
      Source
        .newBuilder("js", scala.io.Source.fromResource("js/nth-prime.mjs").mkString, "nth-prime.mjs")
    ...
    "wasm" ->
      Source
        .newBuilder(
          "wasm",
          ByteSequence
            .create(getClass.getClassLoader().getResourceAsStream("wasm/nth_prime.wasm").readAllBytes()),
          "nth-prime.wasm",
        )
        .build(),
  )
...
class NthPrimeBenchmark:
  ...
  def nthPrime(): Unit = Await.result(system.ask(NthPrimeBenchmarkActors.Start(_)), 30.seconds)
...
object NthPrimeBenchmarkActors:
  ...
  def NthPrimeActor(source: Source)(using blackhole: Blackhole) = Behaviors.setup { ctx =>
      ...
      .receiveMessage { msg =>
        msg match
          case Execute(times, replyTo) =>
            ...
            if times > 1 then
              ctx.self ! Execute(times - 1, replyTo)
            else
              replyTo ! Done
