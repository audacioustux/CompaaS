...
class IncBenchmark:
  ...
  def inc(): Unit = Await.result(system.ask(IncBenchmarkActors.Start(_)), 30.seconds)
...
object IncBenchmarkActors:
  ...
  def IncActor(source: Source)(using blackhole: Blackhole) = Behaviors.setup { ctx =>
