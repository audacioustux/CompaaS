...
object TypedBenchmarkActors {
  ...
  private def echoBehavior(respondTo: ActorRef[Message.type]): Behavior[Message.type] =
    ...
    Behaviors
      .setup[Any] { ctx =>
        ...
        val pairs = (1 to numPairs).map { _ =>
          ctx.spawnAnonymous(
            echoSender(messagesPerPair, ctx.self.narrow[Done], batchSize, props),
            props
          )
        }
    ...
    Behaviors.setup { ctx =>
      Behaviors.receiveMessage {
        case StartPingPong(
          ...
          val (actors, latch) =
            startPingPongActorPairs(ctx, numMessagesPerActorPair, numPairs, dispatcher)
          val startNanoTime = System.nanoTime()
